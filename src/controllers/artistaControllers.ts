...
const createArtista = async (req: Request, res: Response) => {
    try {
        ...
        if (!name || !email || !password || !genre || !description || !image || !banner ) {
        ...
        res.status(201).json({ message: "Artista criado com sucesso.", user: newArtista });
    } catch (error) {
...
const getArtista = async (req: Request, res: Response) =>{
    const artistaId = req.artista?._id;
...
const deleteUser = async (req:Request, res: Response) => {
    ...
    try {
        ...
        if (!artista) {
            return res.status(404).json({ message: 'Artista not found.' });
          }
          
          res.status(200).json({message: "Artista deletado com sucesso" });
    } catch (Error) {
        res.status(500).json({ message: "Ocorreu um erro enquanto se deletava o Artista"})
    }
    
}
export { createArtista, getArtista, listArtista, deleteUser };
